...
if git-diff --cached --name-only | grep -E '\.(cpp|h)$' | xargs grep -nE 'TODO|FIX|DEBUG\s*<<' --exclude-dir=.git --exclude-dir=build --exclude-dir=.vscode; then
    echo "[ PRE-COMMIT ] Error: Found TODO, FIX, or DEBUG in staged files!"
    echo "[ PRE-COMMIT ] Commit blocked."
    exit 1
fi

echo "[ PRE-COMMIT ] No TODO|FIX|DEBUG found."
exit 0
