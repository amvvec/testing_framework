...
do
    if
        !clang - format - i "$file";
then echo "[ PRE-COMMIT ] Error: clang-format failed on $file." exit 1 fi git add "$file" done
        ...
        only |
    grep - E '\.(cpp|h)$' |
    xargs grep - nE 'TODO|FIX|DEBUG\s*<<' --exclude - dir = build-- exclude - dir =.git-- exclude - dir =.vscode;
then echo "[ PRE-COMMIT ] Error: found TODO, FIX, or DEBUG in staged files!" echo
