...
int TestRunner::run_all_tests() {
    ...
    for(const auto& test : tests) {
        ...
        if(!test.fn() || std::find_if(results_.begin(), results_.end(), [&test](const TestResult& r) {
                             return r.group == test.group() && r.name == test.name();
                         }) != results_.end()) {
            continue;
        }
    ...
    auto total_duration =
        std::chrono::duration_cast<std::chrono::milliseconds>(total_end - total_start).count(); // Duration of all tests
...
TestResult TestRunner::run_test(const TestCase& test) {
    ...
    output_(result.passed ? "\033[32m[ OK ]\033[0m " + test.group() + "." + test.name() + " (" +
                                std::to_string(result.duration_ms) + " ms)" + "\n"

                          : "\033[31m[ FAIL ]\033[0m " + test.group() + "." + test.name() + " (" +
                                std::to_string(result.duration_ms) + " ms) - " + result.error_message + "\n");
...
void TestRunner::print_summary(size_t total, size_t passed, size_t failed, long long duration) const {
    output_("=== Summary ===");
