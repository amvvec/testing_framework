...
static void print_summary(size_t total, int passed, int failed, long long total_ms) 
{
    std::cout << "\n=== Summary ===\n";
...
{
    const auto& all_tests = unit::get_all_tests();
    ...
    if (!filter.empty()) 
    {
        std::cout << "[ Filter: " << filter << " → " << tests.size() << " test(s) ]\n";
    }
