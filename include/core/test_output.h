...
private:
    ...
    // // HARDCODE
    // /// @brief Returns shorten path of the file
    // inline std::string_view get_shorten_path(std::string_view file) {
    //     constexpr std::string_view base_path =
    //     "/mnt/d/code/project/test_framework/"; // Hardcode
    //     if(file._Starts_with(base_path)) { // file.starts_with
    //         file.remove_prefix(base_path.size());
    //     }
    //     return file;
    // }

    // // cmake variable PROJECT_ROOT
    // /// @brief Returns shorten path of the file via cmake set variable
    // PROJECT_ROOT
    // // inline std::string_view get_shorten_path(std::string_view file) {
    // //     constexpr std::string_view base_path = PROJECT_ROOT; // TODO: tune
    // the cmake variable PROJECT_ROOT
    // //     if(file._Starts_with(base_path)) { // file.starts_with
    // //         file.remove_prefix(base_path.size());
    // //     }
    // //     return file;
    ...
    // // Only file name
    // // basename("/mnt/d/.../code/.test.cpp"); ---> test.cpp
    // /// @brief Return shorten path of the file
    // inline std::string_view base_name(std::string_view file) {
...
public:
    ...
    void report_failure(std::string_view file, int line, std::string_view label,
        ...
        std::string msg =
            std::string(label) + " failed: " + std::string(details);
        fail(file, line, msg);
    }
    ...
    bool compare(const A& a, const B& b, OP op, std::string_view file, int line,
        ...
        if(!op(a, b)) {
            report_failure(file, line, label,
